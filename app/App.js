...
import { R_SERVER_URL, X_R_SERVER_URL, PROCESS_ETA_EP, RAND_INT, API_AUDIENCE } from '@env';
...
const App = () => {
  ...
  const handleSubmit = async () => {
    const modifiedInputValues = inputValues.map(value => value === '' ? '0' : value);

    if (modifiedInputValues.some(value => Number(value.trim()) < 0)) {
      Alert.alert('Error', 'Please enter a positive value in input boxes');
    } else {
      try {
        console.log('Invoking ' + R_SERVER_URL + PROCESS_ETA_EP + ' ... body: ' + JSON.stringify({ client_id: "r_client", data: [randInt] }))
        ...
        const xMinusRValues = modifiedInputValues.map(value => Number(value) - randInt);
        // console.log(`X_R = ${xMinusRValues}`);

        console.log('Invoking ' + X_R_SERVER_URL + PROCESS_ETA_EP + ' ... body: ' + JSON.stringify({ client_id: "x_r_client", data: xMinusRValues }))
